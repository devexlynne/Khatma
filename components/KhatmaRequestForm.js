...
export default function KhatmaRequestForm({ currentUser = null }) {
  ...
  return <section className="card moroccan-frame khatma-request-card" id="request-khatma">
    ...
    <form onSubmit={submit} className="khatma-request-form">
      ...
      <div className="smart-two-columns">
        <div className="field"><label htmlFor="beneficiary-status">نوع الدعاء</label><select id="beneficiary-status" className="input" value={form.beneficiaryStatus} onChange={update("beneficiaryStatus")}><option value="deceased">لشخص متوفى — رحمه الله</option><option value="living">لشخص حي — حفظه الله</option></select></div>
