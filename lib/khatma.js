...
export function adminOverview() {
  ...
  const users = db.prepare(
    `SELECT u.id, u.full_name, u.email, u.role, u.created_at,
            COUNT(DISTINCT k.id) AS khatmas,
            COUNT(DISTINCT CASE WHEN k.status='completed' THEN k.id END) AS completed_khatmas,
            SUM(CASE WHEN j.status='completed' THEN 1 ELSE 0 END) AS completed_juz
       FROM users u
       LEFT JOIN khatmas k ON k.owner_id = u.id
       LEFT JOIN juz j ON j.khatma_id = k.id
      GROUP BY u.id
