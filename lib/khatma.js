...
export function adminOverview() {
  ...
  const pendingUserApprovals = db.prepare("SELECT COUNT(*) AS count FROM users WHERE approval_status='pending'").get().count || 0;
  ...
  const users = db.prepare(
    `SELECT u.id, u.full_name, u.email, u.role, u.approval_status, u.created_at,
